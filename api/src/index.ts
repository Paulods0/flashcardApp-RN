...
app.delete("/sets/:id", async (req, res) => {
  const { id } = req.params
  const existingSets = await client.db.user_set.filter({ set: id }).getAll()

  if (existingSets.length > 0) {
    const toDelete = existingSets.map((set: SetsRecord) => set.id)
    await client.db.user_set.delete(toDelete)
  }
...
app.post("/usersets", async (req, res) => {
  ...
  const userSet = await client.db.user_set.create({
    user,
    set,
  })
...
app.get("/usersets", async (req, res) => {
  ...
  const sets = await client.db.user_set
    .select(["id", "set.*"])
    .filter({ user: `${user}` })
    .getAll()
